...
export default function AddMember() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const amountPaidValue = parseFloat(formData.amountPaid);
      if (amountPaidValue > 0) {
        await addDoc(collection(db, 'payments'), {
          memberId: memberRef.id,
          memberName: formData.fullName.trim(),
          memberMobile: formData.mobile.trim(),
  ...
  if (loading) {
    return (
      <AdminLayout
        title="Add Member"
        subtitle="Enter member details to create a new membership"
        actions={<Link to="/admin/dashboard" className="btn-secondary text-sm py-2 px-4">← Back to Dashboard</Link>}
      >
        <form className="space-y-6 animate-fade-in-up animate-pulse">
          {/* Photo Upload */}
          <div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex flex-col gap-2 min-w-0 flex-1">
                ...
                <div className="skeleton skeleton-text-sm" style={{ width: '100px' }}></div>
  ...
  return (
    ...
    >
      {success && (
        <div className="mb-6 p-4 bg-green-50 text-green-700 rounded-xl text-center animate-fade-in-up">
          Member created successfully! Redirecting...
        </div>
      )}

      {error && (
        <div className="mb-6 p-4 bg-red-50 text-red-600 rounded-xl text-sm text-center animate-fade-in-up">
          {error}
        </div>
      )}
